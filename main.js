function scroll() {
	...
	t.setTime(t.getTime() + 6e4),
		(document.cookie = `scroll${window.location.pathname}=${$(
			"section"
		).scrollTop()};expires=${t.toUTCString()};path=${
...
console.log(
	...
	$(".image img")
		.next(".cover")
		.each(function () {
			$(this).width() < 0.9 * $("section").width() &&
				($(this).css("cursor", "pointer"),
				$(this).attr({ title: "View image", "data-toggle": "tooltip" }),
				$(this).on("click", function () {
					window.open($(this).prev("img").attr("src"), "_blank");
				}));
		});
