...
$("#calculate").on("click",function(){
    ...
    $(".section.interactive .options .in").each(function(){
        ...
        $(".section.interactive .options .out").each(function(){
            var out_ = $(this)
            $(this).parent().parent().append(line(in_.position().left + in_.outerWidth(false),in_.position().top + in_.outerHeight(false) / 2,out_.position().left + (out_.outerWidth(true) - out_.outerWidth(false)) / 2,out_.position().top + out_.outerHeight(false) / 2))
        })
    })
})
