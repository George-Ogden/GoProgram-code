class Matrix {
    ...
    clip(n, comparison = Matrix.blank(this.rows, this.cols, 1)) {
        //limit the size of a matrix
        this.set(this.map((x, y) => Math.abs(x) > Math.abs(y * n) ? x > 0 ? Math.abs(y * n) : -Math.abs(y * n): x, comparison))
    }

    static clip(matrix, n, comparison = Matrix.blank(this.rows, this.cols, 1)) {
        //initialise new matrix
        let new_matrix = matrix.copy();
        //clip
        new_matrix.clip(n, comparison);
        //return new matrix
        return new_matrix;
    }
    ...
    static flip(matrix, x = true, y = true) {
        ...
        for (let i = 0; i < matrix.rows; i++) {
            for (let j = 0; j < matrix.cols; j++) {
                //flip data
                new_matrix.data[i][j] = matrix.data[y ? matrix.rows - i - 1 : i][x ? matrix.cols - j - 1 : j];
            }
        }
        //return new matrix
        return new_matrix;
